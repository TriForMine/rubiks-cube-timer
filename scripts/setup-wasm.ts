...
async function main() {
	...
	try {
		...
		if (existsSync(wasmPackageJson)) {
			...
			await execCommand("wasm-pack build --target bundler --out-dir pkg-bundler --release", {
				env: buildEnv,
			});
		}
		...
		if (existsSync(wasmFileB)) {
			try {
				...
				await execCommand(`wasm-opt -Oz --enable-mutable-globals ${wasmFileB} -o ${wasmFileB}`, {
					quiet: true,
				});
