  ...
  },
  {
    ...
    criteria: (times: TimeRecord[]) =>
      times.some(
        (t) =>
          t.penalty !== "DNF" &&
          (t.penalty === "+2" ? t.time + 2000 : t.time) < 300000,
      ),
    tier: "bronze",
  },
  {
    ...
    criteria: (times: TimeRecord[]) =>
      times.some(
        (t) =>
          t.penalty !== "DNF" &&
          (t.penalty === "+2" ? t.time + 2000 : t.time) < 180000,
      ),
    tier: "bronze",
  },
  {
    ...
    criteria: (times: TimeRecord[]) =>
      times.some(
        (t) =>
          t.penalty !== "DNF" &&
          (t.penalty === "+2" ? t.time + 2000 : t.time) < 60000,
      ),
    tier: "silver",
  },
  {
    ...
    criteria: (times: TimeRecord[]) =>
      times.some(
        (t) =>
          t.penalty !== "DNF" &&
          (t.penalty === "+2" ? t.time + 2000 : t.time) < 30000,
      ),
    tier: "gold",
  },
  {
    ...
    criteria: (times: TimeRecord[]) =>
      times.some(
        (t) =>
          t.penalty !== "DNF" &&
          (t.penalty === "+2" ? t.time + 2000 : t.time) < 20000,
      ),
    tier: "gold",
  },
  {
    ...
    criteria: (times: TimeRecord[]) =>
      times.some(
        (t) =>
          t.penalty !== "DNF" &&
          (t.penalty === "+2" ? t.time + 2000 : t.time) < 10000,
      ),
    tier: "diamond",
  },
  {
    ...
    name: "Consistent",
    description:
      "Complete 5 solves with less than 5 second difference between fastest and slowest",
    ...
    criteria: (times: TimeRecord[]) => {
      ...
      const adjustedTimes = last5.map((t) =>
  ...
  },
  {
    ...
    criteria: (times: TimeRecord[]) => {
      ...
      return (
        ...
        weeklyProgress.every((day) => day.goalMet)
      );
    },
    tier: "gold",
  },
  {
    ...
    criteria: (times: TimeRecord[]) => {
      ...
      return todayProgress
        ? todayProgress.solveCount >= dailySettings.dailyGoal * 2
        : false;
    },
    tier: "silver",
  },
  {
    ...
    criteria: (times: TimeRecord[]) => {
      ...
      streakStartDate.setDate(
        streakStartDate.getDate() - streakStatus.currentStreak + 1,
      );
      ...
      const validTimesInStreak = timesInStreak.filter(
        (t) => t.penalty !== "DNF",
      );
      const bestTimeInStreak = Math.min(
        ...validTimesInStreak.map((t) =>
          t.penalty === "+2" ? t.time + 2000 : t.time,
        ),
      );
