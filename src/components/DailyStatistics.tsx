...
export function DailyStatistics({ className }: DailyStatisticsProps) {
	...
	const totalWeeklySolves = weeklyProgress.reduce((sum, day) => sum + day.solveCount, 0);
	const averageDailySolves = totalDaysWithSolves > 0 ? totalWeeklySolves / totalDaysWithSolves : 0;

	const bestDayThisWeek = weeklyProgress.reduce((best, day) => {
		...
		<div className={cn("space-y-6", className)}>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<StatCard
					...
					subtitle={<span className="text-xs text-muted-foreground">solves per active day</span>}
				/>
			</div>
			...
			<Card className="shadow-lg border-border/50">
				<CardHeader>
					<div className="flex items-center gap-3">
						...
						<div>
							<CardTitle className="text-lg font-bold">Daily Goal Progress</CardTitle>
						...
						<div className="grid grid-cols-2 gap-4 pt-2 border-t">
							<div className="text-center">
								...
								<div className="text-lg font-bold text-primary">
									{todayProgress.bestTime ? formatTime(todayProgress.bestTime) : "N/A"}
								</div>
							</div>
							<div className="text-center">
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground">Last 7 days of practice</p>
							</div>
						</div>
						<div className="text-right">
							<div className="text-sm text-muted-foreground">Goal Rate</div>
							<div className="text-lg font-bold text-primary">{weeklyGoalRate.toFixed(0)}%</div>
						</div>
					</div>
				</CardHeader>
				<CardContent className="space-y-6">
					...
					<div className="grid grid-cols-3 gap-4">
						<div className="text-center p-3 bg-muted/30 rounded-lg">
							...
							<div className="text-xl font-bold text-primary">{totalDaysWithSolves}/7</div>
						</div>
						<div className="text-center p-3 bg-muted/30 rounded-lg">
						...
						<div className="text-center p-3 bg-muted/30 rounded-lg">
							...
							<div className="text-xl font-bold text-primary">{totalWeeklySolves}</div>
						</div>
					</div>
									...
									<div key={day.date} className="text-center">
										...
										<div
											...
											title={`${day.date}: ${day.solveCount} solves${
												day.bestTime ? ` (best: ${formatTime(day.bestTime)})` : ""
											}`}
										>
											<div className="font-bold">{day.solveCount}</div>
											{day.bestTime && (
												<div className="text-xs opacity-75">{formatTime(day.bestTime)}</div>
											)}
											{isToday && (
					...
					{bestDayThisWeek?.bestTime && (
						<div className="border-t pt-4">
							<h4 className="font-semibold text-sm mb-2">Best Performance This Week</h4>
			...
			<Card className="shadow-lg border-border/50">
				<CardHeader>
					<div className="flex items-center gap-3">
						...
						<div>
							<CardTitle className="text-lg font-bold">Streak Analysis</CardTitle>
							<p className="text-sm text-muted-foreground">
