...
import { Copy, Eye, RefreshCw, Shuffle } from "lucide-react";
...
export function ScrambleDialog({ scramble, onNewScramble, children }: ScrambleDialogProps) {
				...
				<div className="space-y-6">
					...
					<div className="bg-amber-50 dark:bg-amber-900 rounded-lg p-4 border border-amber-200 dark:border-amber-700">
						...
						<p className="text-sm text-amber-800 dark:text-amber-200 mb-2">
							After applying the scramble, your cube should match the pattern above. Use this to
							verify you&apos;ve executed the scramble correctly.
						</p>
						<div className="text-xs text-amber-700 dark:text-amber-300">
							<strong>Tip:</strong> Compare the colors on each visible face with your physical cube
							to confirm accuracy.
						</div>
					</div>
						...
						<div className="grid grid-cols-2 gap-4 text-sm">
							<div className="space-y-2">
								<div className="flex justify-between">
									...
									<span className="font-mono font-bold">{scrambleMoves.length}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Face turns:</span>
									<span className="font-mono font-bold">
										{
											scrambleMoves.filter((move) => !move.includes("'") && !move.includes("2"))
												.length
										}
									</span>
								</div>
							...
							<div className="space-y-2">
								<div className="flex justify-between">
									...
									<span className="font-mono font-bold">
										{scrambleMoves.filter((move) => move.startsWith("R")).length}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">U moves:</span>
									<span className="font-mono font-bold">
										{scrambleMoves.filter((move) => move.startsWith("U")).length}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">F moves:</span>
									<span className="font-mono font-bold">
										{scrambleMoves.filter((move) => move.startsWith("F")).length}
									</span>
								</div>
								<div className="flex justify-between">
									...
									<span className="font-mono font-bold">
										{
											scrambleMoves.filter(
												(move) =>
													move.startsWith("L") || move.startsWith("D") || move.startsWith("B")
											).length
										}
									</span>
								</div>
					...
					<div className="flex flex-wrap gap-3 pt-4 border-t">
						<Button
							variant="default"
							onClick={handleCopy}
							className={cn("flex-1 min-w-[120px]", copied && "bg-green-600 hover:bg-green-700")}
						>
							{copied ? (
								<>
									<Eye className="w-4 h-4 mr-2" />
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-2 gap-4">
							<div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 border">
								<h4 className="font-medium text-sm mb-2">Most Affected Faces</h4>
								<div className="space-y-1 text-xs">
									{Object.entries({
										"Right (R)": scrambleMoves.filter((move) => move.startsWith("R")).length,
										"Up (U)": scrambleMoves.filter((move) => move.startsWith("U")).length,
										"Front (F)": scrambleMoves.filter((move) => move.startsWith("F")).length,
										"Left (L)": scrambleMoves.filter((move) => move.startsWith("L")).length,
										"Down (D)": scrambleMoves.filter((move) => move.startsWith("D")).length,
										"Back (B)": scrambleMoves.filter((move) => move.startsWith("B")).length,
									})
										.sort(([, a], [, b]) => b - a)
										.slice(0, 3)
										.map(([face, count]) => (
											<div key={face} className="flex justify-between">
												...
												<span className="font-mono font-bold">{count} moves</span>
											</div>
										))}
								</div>
							</div>
							...
							<div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 border">
								<h4 className="font-medium text-sm mb-2">Scramble Complexity</h4>
								<div className="space-y-1 text-xs">
									<div className="flex justify-between">
					...
					<div className="bg-blue-50 dark:bg-blue-900 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
						...
						<ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
							<li>• Hold the cube with white face on top, green face facing you</li>
							<li>
								• Execute moves exactly as written (R = clockwise, R&apos; = counterclockwise)
							</li>
							<li>• R2 means turn the right face 180 degrees (two quarter turns)</li>
							<li>• Take your time with the scramble - accuracy is more important than speed</li>
							<li>• Compare your cube to the preview above after completing all moves</li>
						</ul>
					</div>
				</div>
			</DialogContent>
