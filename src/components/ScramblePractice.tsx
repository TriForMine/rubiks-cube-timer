...
export function ScramblePractice({ className = "" }: ScramblePracticeProps) {
	...
	const generateNewScramble = useCallback(() => {
		...
		switch (settings.mode) {
			case "random": {
				newScramble = generateScramble(settings.scrambleLength);
				break;
			}
			case "algorithm": {
				newScramble = generateAlgorithmPractice(settings.algorithmType);
				break;
			}
			case "difficulty": {
				const config = DIFFICULTY_CONFIGS[settings.difficultyLevel];
				newScramble = generateScramble(config.length);
				break;
			}
		}
			...
			<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
				<div>
					...
					<p className="text-gray-600 dark:text-gray-400 mt-1">
						Master cube scrambles with interactive 3D visualization and step-by-step guidance
					</p>
				</div>
				...
				<div className="flex flex-wrap items-center gap-3">
					...
					<Button variant="default" size="sm" onClick={generateNewScramble}>
						...
						>
							...
							<TabsContent value="algorithm" className="space-y-4 mt-4">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										<label htmlFor="algorithm-type" className="block text-sm font-medium mb-2">
											Algorithm Type
										</label>
										<select
											id="algorithm-type"
											...
											onChange={(e) =>
												setSettings((prev) => ({
													...prev,
													algorithmType: e.target.value as keyof typeof ALGORITHM_PATTERNS,
												}))
											}
											className="w-full p-2 border rounded text-sm bg-white dark:bg-gray-700"
										>
									...
									<div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded border">
										...
										<div className="text-xs text-blue-600 dark:text-blue-300">
											...
											{settings.algorithmType === "PLL" && "Practice permuting the last layer"}
											{settings.algorithmType === "F2L" && "Master first two layers efficiency"}
										</div>
									</div>
								</div>
							</TabsContent>
									...
									{Object.entries(DIFFICULTY_CONFIGS).map(([level, config]) => (
										<button
											...
											onClick={() =>
												setSettings((prev) => ({
													...prev,
													difficultyLevel: level as PracticeSettings["difficultyLevel"],
												}))
											}
										>
							...
							<CardContent>
								<div className="space-y-2">
									...
									<div className="text-sm text-center text-gray-600 dark:text-gray-400">
										{MOVE_EXPLANATIONS[currentMove as keyof typeof MOVE_EXPLANATIONS] ||
											"Unknown move"}
									</div>
								</div>
							</CardContent>
