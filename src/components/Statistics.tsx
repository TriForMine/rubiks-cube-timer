...
export function Statistics({ times }: StatisticsProps) {
	...
	const stats = useMemo(() => {
		...
		return {
			...
			ao12: validTimes.length >= 12 ? calculateAverage(validTimes.slice(-12)) : 0,
			ao100: validTimes.length >= 100 ? calculateAverage(validTimes.slice(-100)) : 0,
	...
	const successRate = useMemo(() => {
		const rate = stats.totalSolves > 0 ? (stats.validCount / stats.totalSolves) * 100 : 0;
		const recent10 = times.slice(-10);
		const recentSuccessRate =
			recent10.length > 0
				? (recent10.filter((t) => t.penalty !== "DNF").length / recent10.length) * 100
				: 0;
	...
	if (times.length === 0) {
		return (
			<div className="text-center py-12">
				...
				<p className="text-muted-foreground">Start solving to see your statistics and progress!</p>
			</div>
		);
	}
							...
							<CardContent className="space-y-4">
								<div className="grid grid-cols-2 gap-4">
									<div>
										...
										<p className="text-xl font-bold">{formatTime(stats.best)}</p>
									</div>
									<div>
										<p className="text-sm text-muted-foreground">Worst Single</p>
										<p className="text-xl font-bold">{formatTime(stats.worst)}</p>
									</div>
									<div>
									...
									<div>
										...
										<p className="text-xl font-bold">{formatTime(stats.ao12)}</p>
									</div>
									{stats.ao100 > 0 && (
										<div className="col-span-2">
											<p className="text-sm text-muted-foreground">Ao100</p>
											<p className="text-xl font-bold">{formatTime(stats.ao100)}</p>
										</div>
									)}
								</div>
							</CardContent>
								...
								<div className="space-y-3">
									<div className="flex justify-between items-center">
										...
										<div className="flex items-center gap-2">
											...
											<Badge variant="secondary" className="bg-green-100 text-green-800">
												{((stats.validCount / stats.totalSolves) * 100).toFixed(1)}%
											</Badge>
										</div>
									</div>
									{stats.plusTwoCount > 0 && (
										<div className="flex justify-between items-center">
											...
											<div className="flex items-center gap-2">
												<span className="font-medium">{stats.plusTwoCount}</span>
												<Badge variant="secondary" className="bg-yellow-100 text-yellow-800">
													{((stats.plusTwoCount / stats.totalSolves) * 100).toFixed(1)}%
												</Badge>
											</div>
										</div>
									)}
										...
										<div className="flex justify-between items-center">
											...
											<div className="flex items-center gap-2">
												...
												<Badge variant="secondary" className="bg-red-100 text-red-800">
													{((stats.dnfCount / stats.totalSolves) * 100).toFixed(1)}%
												</Badge>
											</div>
										</div>
									)}
