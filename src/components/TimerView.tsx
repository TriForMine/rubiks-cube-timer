...
const MotivationalMessage = memo(
	({
		...
		const message = useMemo(() => {
			...
			if (wasLastSolvePB) {
				if (lastSolveTime < 10000) return `NEW PB! ${formattedTime} - Incredible! 🏆⚡`;
				if (lastSolveTime < 20000) return `NEW PB! ${formattedTime} - Outstanding! 🏆💪`;
				if (lastSolveTime < 30000) return `NEW PB! ${formattedTime} - Well done! 🏆📈`;
				return `NEW PB! ${formattedTime} - Great achievement! 🏆🎯`;
			}

			if (lastSolveTime < 10000) return `Amazing! ${formattedTime} - You're on fire! 🔥`;
			if (lastSolveTime < 20000) return `Great solve! ${formattedTime} - Keep it up! 💪`;
			if (lastSolveTime < 30000) return `Nice work! ${formattedTime} - Getting better! 📈`;
		...
		return (
			<div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 px-4">
				<Button
					variant="outline"
					size="lg"
					onClick={() => onNewScramble().catch(console.error)}
...
function TimerViewComponent({
	...
	const [completedTimeRecord, setCompletedTimeRecord] = useState<TimeRecord | null>(null);
	...
	const stopTimer = useCallback(() => {
		...
		if (startTimeRef.current) {
			...
			const validTimes = existingTimesRef.current.filter((t) => t.penalty !== "DNF");
	...
	const handleDeleteLastTime = useCallback(() => {
		if (!isRunning && existingTimes.length > 0) {
			const lastTime = existingTimes[existingTimes.length - 1];
			if (confirm("Are you sure you want to delete the last time?")) {
				onDeleteTime(lastTime.id);
			}
		}
	}, [isRunning, existingTimes, onDeleteTime]);
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			// The ScrambleDialog now handles preventing escape key bubbling when open
	...
	}, [isRunning, isReady, startTimer, stopTimer, handleNewScramble, handleDeleteLastTime]);
			...
			<div className="flex-1 flex items-center justify-center p-2 sm:p-4">
				<div className="w-full max-w-6xl">
					<div className="text-center space-y-4 sm:space-y-6 md:space-y-8">
						...
						<div className="relative flex flex-col items-center justify-center py-16 overflow-hidden">
							...
							<div
								className={cn(
									...
									isRunning && "bg-gradient-to-r from-green-500/10 via-transparent to-green-500/10",
									isReady && "bg-gradient-to-r from-amber-500/10 via-transparent to-amber-500/10",
									spacePressed && "bg-gradient-to-r from-red-500/10 via-transparent to-red-500/10",
									!isRunning &&
										!isReady &&
										!spacePressed &&
							...
							<div className="relative z-10 flex flex-col items-center justify-center">
								<div
									...
									className={cn(
										...
										!isRunning && !isReady && !spacePressed && "text-foreground scale-100",
										isRunning && "drop-shadow-[0_0_20px_rgba(34,197,94,0.3)]",
										isReady && "drop-shadow-[0_0_20px_rgba(245,158,11,0.3)]",
										spacePressed && "drop-shadow-[0_0_20px_rgba(239,68,68,0.3)]"
									)}
									style={{
								...
								<div className="flex items-center space-x-4 mt-6">
									...
									<div
										className={cn(
											...
											isRunning && "text-green-500 dark:text-green-400 animate-pulse",
											isReady && "text-amber-500 dark:text-amber-400",
											spacePressed && "text-red-500 dark:text-red-400",
											!isRunning && !isReady && !spacePressed && "text-muted-foreground"
										)}
									>
										{isRunning ? "SOLVING" : isReady ? "READY" : spacePressed ? "HOLD" : "IDLE"}
									</div>
								</div>
							</div>
						</div>
						...
						<MotivationalMessage lastSolveTime={lastSolveTime} wasLastSolvePB={wasLastSolvePB} />
							...
							<div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 text-xs sm:text-sm text-muted-foreground animate-fade-in px-4">
								<div className="flex items-center gap-2">
									<div className="w-9 h-9 sm:w-8 sm:h-8 bg-muted rounded-lg flex items-center justify-center">
										<span className="text-[10px] sm:text-xs font-mono font-semibold">SPC</span>
									</div>
								...
								<div className="flex items-center gap-2">
									<div className="w-9 h-9 sm:w-8 sm:h-8 bg-muted rounded-lg flex items-center justify-center">
										<span className="text-[10px] sm:text-xs font-mono font-semibold">N</span>
									</div>
									<span className="text-center sm:text-left">New scramble</span>
								</div>
								<div className="flex items-center gap-2">
									<div className="w-9 h-9 sm:w-8 sm:h-8 bg-muted rounded-lg flex items-center justify-center">
										<span className="text-[10px] sm:text-xs font-mono font-semibold">DEL</span>
									</div>
									<span className="text-center sm:text-left">Delete last</span>
								</div>
							</div>
						)}
