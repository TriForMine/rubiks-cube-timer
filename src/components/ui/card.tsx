  ...
  "rounded-2xl border bg-card text-card-foreground transition-all duration-200",
  {
    variants: {
      variant: {
        ...
        gradient:
          "bg-gradient-to-br from-card via-card to-muted/50 shadow-md hover:shadow-lg",
        interactive:
...
const cardFooterVariants = cva(
  "flex items-center border-t bg-muted/30 rounded-b-2xl",
  {
    variants: {
      padding: {
        none: "p-0",
...
const CardFooter = React.forwardRef<HTMLDivElement, CardFooterProps>(
  ({ className, padding, variant, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(cardFooterVariants({ padding, variant, className }))}
      {...props}
    />
  ),
);
            ...
            {icon && (
              <div
                className={cn(
                  ...
                  compact ? "[&_svg]:size-3" : "[&_svg]:size-4",
                )}
              >
                {icon}
              </div>
