...
import { CubeColor, getDisplayHex, initWasm, isWasmInitialized, type WasmCube } from "./cube-wasm";
...
export class Cube3DRenderer {
	...
	private faceMapping: Record<keyof CubeState, { normal: Vector3; pieces: CubePiece[] }> = {
	...
	constructor(canvas: HTMLCanvasElement, initialState?: CubeState) {
		...
		if (!isWasmInitialized()) {
			initWasm().catch(() => {
				console.warn("WASM initialization failed in 3D renderer - using fallback");
			});
		}
			...
			try {
				...
				materials.forEach((m) => {
					...
					textureProperties.forEach((prop) => {
						...
						const texture = material[prop as keyof DisposableMaterial] as Texture | null;
						if (texture && typeof texture.dispose === "function") {
							texture.dispose();
							(material as unknown as Record<string, Texture | null>)[prop] = null;
						}
					});
