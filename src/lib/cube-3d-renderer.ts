...
export class Cube3DRenderer {
	...
	private animate = () => {
		...
		if (this.animationState) {
			...
			const eased = t < 0.8 ? this.easeInOutCubic(t) : this.easeOutBack(t);
	...
	public animateMove(move: string, duration = 500): Promise<void> {
	...
	public async animateScramble(scramble: string, delay = 600) {
		const moves = scramble.trim().split(/\s+/).filter(Boolean);
		for (const mv of moves) {
			await this.animateMove(mv, delay - 200);
