...
export function generateScramble(length: number = 20): string {
	...
	while (scramble.length < length && attempts < maxAttempts) {
		...
		const previousMove = scramble.length > 0 ? scramble[scramble.length - 1] : null;
		const beforePreviousMove = scramble.length > 1 ? scramble[scramble.length - 2] : null;
...
export function generateAlgorithmPractice(type: keyof typeof ALGORITHM_PATTERNS): string {
