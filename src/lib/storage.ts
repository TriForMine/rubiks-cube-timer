...
export function loadDailySettings(): DailySettings {
	...
	try {
		const saved = localStorage.getItem(`${STORAGE_KEYS.DAILY_PROGRESS}-settings`);
		if (saved) {
...
export function saveDailySettings(settings: DailySettings): void {
	...
	try {
		localStorage.setItem(`${STORAGE_KEYS.DAILY_PROGRESS}-settings`, JSON.stringify(settings));
	} catch (error) {
...
export function getCurrentStreakStatus(): {
	...
	const todayProgress = streakData.dailyProgress.find((p) => p.date === today) || null;
...
export function getWeeklyProgress(): DailyProgress[] {
	...
	for (let i = 6; i >= 0; i--) {
		...
		const existingProgress = streakData.dailyProgress.find((p) => p.date === dateString);
		weeklyProgress.push(
			existingProgress || {
				date: dateString,
				solveCount: 0,
