...
export function formatTime(milliseconds: number, settings?: TimeFormatSettings): string {
...
export function calculateAverageOf(times: number[], count: number): number | null {
	if (times.length < count) return null;
